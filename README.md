...
В корне репозитория находится Blueprint [`render.yaml`](render.yaml), который создаёт
два web‑сервиса (`appetit-web` и `appetit-api`), PostgreSQL и постоянный диск для
загруженных изображений и логов API.

1. Загрузите репозиторий в GitHub/GitLab и в Render выберите **New → Blueprint**.
2. Подключите репозиторий и подтвердите ресурсы из `render.yaml`.
3. До первого запуска заполните секреты со значением `sync: false`: ключ Яндекс
   Карт, имя и токен Telegram‑бота, а при использовании уведомлений — Resend и
   Twilio. Неиспользуемые интеграции можно оставить пустыми.
4. Если Render назначил сервисам другие имена, замените
   `NEXT_PUBLIC_API_BASE` и `PUBLIC_ORIGIN` фактическими HTTPS‑адресами и заново
   разверните web‑сервис: публичные переменные Next.js встраиваются при сборке.
5. Для Telegram Login добавьте домен web‑сервиса в BotFather. Для собственного
   домена также обновите обе URL‑переменные.

API при старте синхронизирует Prisma‑схему с управляемой базой. Диск API должен
оставаться подключённым к `/var/data`, иначе загруженные через админку изображения
будут потеряны при следующем деплое. План API в Blueprint не бесплатный именно
потому, что постоянный диск необходим для сохранения загрузок.
...
## Обзор структуры и рекомендации
